...
public class QueryController {
    ...
    public String processAddQueryForm(@ModelAttribute @Valid Query newQuery,
    ...
    public String displayEditQueryForm(Model model, @PathVariable int queryId) {
        ...
        model.addAttribute("query", queryDao.findOne(queryId));
        model.addAttribute("categories", categoryDao.findAll());
        return "queries/edit";
    }

    @RequestMapping(value = "edit/{queryId}", method = RequestMethod.POST)
    public String processEditForm(Model model, @PathVariable int queryId,
                                  @ModelAttribute  @Valid Query newQuery,
                                  @RequestParam int categoryId,
                                  Errors errors) {
        ...
        Query editedQuery = queryDao.findOne(queryId);
        editedQuery.setTitle(newQuery.getTitle());
        editedQuery.setBody(newQuery.getBody());
        editedQuery.setCategory(categoryDao.findOne(categoryId));
